...
$parentModule = 'UserRightsAssignment'
if (-not $ModuleName) { $ModuleName = 'UserRightsAssignment.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\UserRightsAssignment'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\UserRightsAssignment.psd1"
$prereqHash = @{
